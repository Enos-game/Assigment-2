...
public class Gelateria implements TakeAwayBill{
    ...
    public double getOrderPrice(List<MenuItem> itemsOrder, 
        User user) throws BillException{
        double Cibo = 0.0;
        double Bevande = 0.0;
        int nrGelati = 0;
        ...
        if(user.getAnnoDiNascita()>=2003 && OrdiniRegalati <= 10) {
            if((first.getData().getHours() == 18 || 
                first.getData().getHours() == 19) 
                && (first.getData().getMinutes() >= 0 
                || first.getData().getMinutes() <= 59) ) {
        ...
        if(itemsOrder.size() > 30) {
            throw new BillException("troppi elementi nell'ordine effettuato");
        }
